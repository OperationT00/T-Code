...
```bash
cd /Users/itwanger/Documents/GitHub/tcode
mvn -q clean package -DskipTests
```

产物：`target/t-code-1.0-SNAPSHOT.jar`，所有用例都基于这个 jar 启动。
...
```bash
TCODE_RENDERER=lanterna java -jar target/t-code-1.0-SNAPSHOT.jar
```
...
**预期**：
- stderr 出现 `⚠️ TCODE_TUI=true 已废弃，请改用 TCODE_RENDERER=lanterna`
...
```bash
TCODE_RENDERER=plain java -jar target/t-code-1.0-SNAPSHOT.jar
```
...
```bash
NO_COLOR=1 java -jar target/t-code-1.0-SNAPSHOT.jar
```
...
```bash
TCODE_RENDERER=weird java -jar target/t-code-1.0-SNAPSHOT.jar
```

**预期**：stderr `⚠️ 未识别的 TCODE_RENDERER='weird'，回退到 inline`，正常进 inline 模式。
...
```bash
TERM=dumb java -jar target/t-code-1.0-SNAPSHOT.jar
```
...
```
*
 t-code  idle  glm-5.1  ctx 0/200.0k  HITL OFF  MCP 4/4  Skill 2/2
...
**输入**：
```
把 ROADMAP.md 第一行改成 "# t-code 路线图（v1.0 测试）"
```
...
│   [6] 配置文件: ~/.tcode/config.json (只读视图，编辑请用编辑器)
...
## 23. TCODE_NO_STATUSBAR 禁用状态栏

```bash
TCODE_NO_STATUSBAR=true java -jar target/t-code-1.0-SNAPSHOT.jar
```
...
- 旧 Windows cmd / PuTTY 等不适合 ANSI 光标控制的终端：用 `TCODE_NO_STATUSBAR=true` 即可
