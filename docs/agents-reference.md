...
1. `~/.tcode/config.json` 中对应 provider 的 `apiKey`
...
系统属性 > 环境变量/.env > 默认值：`TCODE_LOG_DIR`(~/.tcode/logs) / `TCODE_LOG_LEVEL`(INFO) / `TCODE_LOG_MAX_HISTORY`(7) / `TCODE_LOG_MAX_FILE_SIZE`(10MB) / `TCODE_LOG_TOTAL_SIZE_CAP`(100MB)

### ReAct/SubAgent Budget Config

系统属性 > 默认值：`tcode.react.token.budget`(Integer.MAX_VALUE) / `tcode.react.stagnation.window`(3) / `tcode.react.hard.max.iterations`(50)
...
系统属性 > 默认值：`tcode.llm.connect.timeout.seconds`(60) / `tcode.llm.read.timeout.seconds`(300) / `tcode.llm.write.timeout.seconds`(60) / `tcode.llm.call.timeout.seconds`(600)
...
1. 用户级：`~/.tcode/mcp.json`
2. 项目级：`.tcode/mcp.json`
...
- shared 模式 close_page 只允许关闭 t-code 创建的 tab

### Skill System

- 三层加载：jar 内置 < 用户级 ~/.tcode/skills/ < 项目级 .tcode/skills/
...
- 环境变量：`TCODE_RENDERER=inline|lanterna|plain`
- `TCODE_TUI=true`(旧) → lanterna + deprecation 提示
- `TCODE_NO_STATUSBAR=true`：禁用底部状态栏
...
- `TCODE_LSP_ENABLED=false` 关闭

### Git Side-History Snapshot (Phase 18)

- side-git 在 ~/.tcode/snapshots/ 维护独立仓库（JGit，不依赖系统 git）
...
- 覆盖优先级：jar 内置 < 用户级 ~/.tcode/prompts/ < 项目级 .tcode/prompts/
...
# TCODE_LOG_MAX_FILE_SIZE=10MB
# TCODE_LOG_TOTAL_SIZE_CAP=100MB
# TCODE_SNAPSHOT_ENABLED=true
# TCODE_SNAPSHOT_MAX=50
# TCODE_SNAPSHOT_EXCLUDES=.git,.tcode/snapshots,target,node_modules,dist,.idea,*.class,*.jar
# TCODE_SNAPSHOT_DIR=/Users/yourname/.tcode/snapshots
# TCODE_TUI=true
# NO_TUI=true
```
