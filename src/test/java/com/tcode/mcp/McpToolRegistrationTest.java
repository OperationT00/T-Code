...
class McpToolRegistrationTest {
    ...
    private static void withAuditDir(Path tempDir, ThrowingRunnable body) throws Exception {
        String previous = System.getProperty("tcode.audit.dir");
        System.setProperty("tcode.audit.dir", tempDir.resolve("audit").toString());
        try {
            body.run();
        } finally {
            if (previous == null) {
                System.clearProperty("tcode.audit.dir");
            } else {
                System.setProperty("tcode.audit.dir", previous);
            }
        }
    }
