...
class TerminalMarkdownRendererTest {
    static {
        System.setProperty("tcode.render.color", "false");
    }
    ...
    void fallsBackToKeyValueLayoutForLongTwoColumnTable() {
        String markdown = """
                | 目录名 | 说明 |
                | --- | --- |
                | src/main/java/com/tcode | 这里存放 TCode 的主要 Java 源码实现与相关模块 |
                """;

        String rendered = TerminalMarkdownRenderer.render(markdown);

        assertTrue(rendered.contains("目录名 / 说明"));
        assertTrue(rendered.contains("- src/main/java/com/tcode"));
        assertTrue(rendered.contains("这里存放 TCode 的主要 Java 源码实现与相关模块"));
    }
}
