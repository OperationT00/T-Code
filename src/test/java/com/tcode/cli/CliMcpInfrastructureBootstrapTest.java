...
class CliMcpInfrastructureBootstrapTest {

    @Test
    void createsDefaultChromeDevtoolsMcpConfigWhenMissing(@TempDir Path tempHome) throws Exception {
        CliMcpInfrastructure.McpConfigBootstrapResult result =
                CliMcpInfrastructure.ensureDefaultMcpConfig(tempHome);

        Path config = tempHome.resolve(".tcode").resolve("mcp.json");
