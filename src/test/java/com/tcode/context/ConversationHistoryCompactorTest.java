...
class ConversationHistoryCompactorTest {
    ...
    @Test
    void compactNowCompactsBelowTriggerWhenHistoryHasEnoughTurns() {
        StubCompactor c = new StubCompactor("FORCED SUMMARY", 2);
        List<LlmClient.Message> history = new ArrayList<>();
        history.add(LlmClient.Message.system("system"));
        for (int i = 0; i < 5; i++) {
        ...
        assertTrue(history.get(1).content().contains("FORCED SUMMARY"));
        assertTrue(history.get(3).content().startsWith("Q3"));
    }

    @Test
    void compactNowPassesFocusToFocusedSummarizer() {
        FocusCompactor c = new FocusCompactor("FOCUSED SUMMARY", 2);
        List<LlmClient.Message> history = new ArrayList<>();
        history.add(LlmClient.Message.system("system"));
        for (int i = 0; i < 5; i++) {
            history.add(LlmClient.Message.user("Q" + i));
        ...
        boolean compacted = c.compactNow(history, "keep failures");

        assertTrue(compacted);
        assertEquals("keep failures", c.focus);
        assertTrue(history.get(1).content().contains("FOCUSED SUMMARY"));
    }

    @Test
    void summaryPromptRequestsStructuredMarkdownSections() {
        RecordingLlmClient llm = new RecordingLlmClient("""
                ...
                run tests
                """);
        ConversationHistoryCompactor c = new ConversationHistoryCompactor(llm, 2);
        List<LlmClient.Message> history = new ArrayList<>();
        history.add(LlmClient.Message.system("system"));
        for (int i = 0; i < 5; i++) {
            history.add(LlmClient.Message.user("Q" + i));
        ...
        assertTrue(c.compactNow(history, "keep failed tests"));

        String prompt = llm.lastMessages.get(1).content();
        assertTrue(prompt.contains("Output exactly these Markdown sections"));
        ...
        assertTrue(prompt.contains("## Key Decisions"));
        assertTrue(prompt.contains("## Open Issues"));
        assertTrue(prompt.contains("## Read Files"));
        assertTrue(prompt.contains("## Modified Files"));
        assertTrue(prompt.contains("## Next Steps"));
        assertTrue(prompt.contains("keep failed tests"));
    }

    @Test
    void compactNowStillSkipsWhenUserTurnsTooFew() {
        StubCompactor c = new StubCompactor("MOCK SUMMARY", 3);
        List<LlmClient.Message> history = new ArrayList<>();
        history.add(LlmClient.Message.system("system"));
        history.add(LlmClient.Message.user("Q0"));
        history.add(LlmClient.Message.assistant("A0"));
        history.add(LlmClient.Message.user("Q1"));
    ...
    private static final class FocusCompactor extends ConversationHistoryCompactor {
        private final String mockSummary;
        private String focus;

        FocusCompactor(String mockSummary, int retainRecent) {
            super(null, retainRecent);
            this.mockSummary = mockSummary;
        }

        @Override
        protected String summarize(List<LlmClient.Message> messages, String focus) {
            this.focus = focus;
            return mockSummary;
        }
    }
    ...
    private static final class RecordingLlmClient implements LlmClient {
        ...
        private List<LlmClient.Message> lastMessages = List.of();
        ...
        public ChatResponse chat(List<Message> messages, List<Tool> tools) {
            ...
            return new ChatResponse("assistant", response, null, 10, 10);
        }
