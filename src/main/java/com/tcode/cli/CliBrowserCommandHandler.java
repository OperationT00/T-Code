...
final class CliBrowserCommandHandler {

    private CliBrowserCommandHandler() {
    }
    ...
    private static String browserAutoConnect(BrowserSession browserSession,
        ...
        McpServer server = mcpServerManager.server("chrome-devtools");
        if (server == null) {
            return "❌ 未配置 chrome-devtools MCP server，请先检查 ~/.tcode/mcp.json";
        }
    ...
    private static String chromeLaunchHelp(int port) {
        return """
                请先用调试端口启动 Chrome：
                  macOS: open -na "Google Chrome" --args --remote-debugging-port=%d --user-data-dir=/tmp/tcode-chrome-profile
                  Windows: start chrome.exe --remote-debugging-port=%d --user-data-dir=%%TEMP%%\\tcode-chrome-profile
                  Linux: google-chrome --remote-debugging-port=%d --user-data-dir=/tmp/tcode-chrome-profile
