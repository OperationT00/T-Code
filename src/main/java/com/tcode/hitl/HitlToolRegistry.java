 ...
 * allow / policy-deny / error，HITL 审批与策略拦截共用同一份 ~/.tcode/audit/ 文件。
 */
public class HitlToolRegistry extends ToolRegistry {

    private final HitlHandler hitlHandler;
    private HitlLifecycleListener hitlLifecycleListener = HitlLifecycleListener.NO_OP;
    ...
    public ToolOutput executeToolOutput(String name, String argumentsJson) {
        return executeWithLifecycle(name, argumentsJson, () -> executeWithHitl(name, argumentsJson));
    }

    private ToolOutput executeWithHitl(String name, String argumentsJson) {
    ...
    public void setHitlLifecycleListener(HitlLifecycleListener listener) {
        this.hitlLifecycleListener = listener == null ? HitlLifecycleListener.NO_OP : listener;
    }

    private void notifyRequested(ApprovalRequest request) {
        try {
            hitlLifecycleListener.onRequested(request);
        } catch (Exception ignored) {
            // HITL 生命周期事件是观察信号，不能阻断审批主路径。
        }
    }

    private void notifyResolved(ApprovalRequest request, ApprovalResult result) {
        try {
            hitlLifecycleListener.onResolved(request, result);
        } catch (Exception ignored) {
            // HITL 生命周期事件是观察信号，不能阻断审批主路径。
        }
    }
