...
public record SnapshotConfig(
    ...
    public static SnapshotConfig fromEnvironment() {
        boolean enabled = readBoolean("tcode.snapshot.enabled", "TCODE_SNAPSHOT_ENABLED", true);
        Path root = Path.of(readString("tcode.snapshot.dir", "TCODE_SNAPSHOT_DIR",
                Path.of(System.getProperty("user.home"), ".tcode", "snapshots").toString()));
        int max = readInt("tcode.snapshot.max", "TCODE_SNAPSHOT_MAX", 50);
        List<String> excludes = mergeExcludes(readString("tcode.snapshot.excludes", "TCODE_SNAPSHOT_EXCLUDES", ""));
