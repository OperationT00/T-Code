 ...
 * 1. 系统属性：{@code tcode.react.token.budget} / {@code tcode.react.stagnation.window} /
 *    {@code tcode.react.hard.max.iterations}
 ...
 * 场景（CI / 自动化批跑）通过 {@code -Dtcode.react.token.budget=N} 显式启用。
...
public class AgentBudget {
    ...
    public static AgentBudget fromLlmClient(LlmClient llmClient) {
        ...
        // 显式 -Dtcode.react.token.budget=N 仍可启用硬预算，覆盖默认。
        return new AgentBudget(
                readIntProperty("tcode.react.token.budget", Integer.MAX_VALUE),
                readIntProperty("tcode.react.stagnation.window", DEFAULT_STAGNATION_WINDOW),
                readIntProperty("tcode.react.hard.max.iterations", DEFAULT_HARD_MAX_ITERATIONS)
        );
    }
