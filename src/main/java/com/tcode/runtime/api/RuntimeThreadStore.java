...
public class RuntimeThreadStore implements AutoCloseable {
    ...
    public static Path defaultDbPath() {
        String configured = System.getProperty("tcode.runtime.dir");
        if (configured == null || configured.isBlank()) {
            configured = System.getenv("TCODE_RUNTIME_DIR");
        }
        if (configured == null || configured.isBlank()) {
            configured = Path.of(System.getProperty("user.home"), ".tcode", "runtime").toString();
        }
    ...
    public synchronized String createThread() {
        ...
        try (PreparedStatement ps = connection.prepareStatement("""
            ...
            ps.executeUpdate();
            appendEvent(id, "thread.created", RuntimeEventPayloads.threadCreated(id));
            return id;
