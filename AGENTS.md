...
Tool calling 治理层已补齐基础工程化能力：`ToolExecutionPipeline` 会在执行前按工具 JSON schema 做轻量参数校验（object / required / primitive type / enum），无效 JSON、缺必填或类型错误会返回 `INVALID_ARGUMENTS`，不会进入真实 executor；`ToolOutput` 携带 `ToolCallStatus`、`ToolErrorCode`、retryable、耗时和 attempts，旧 `executeTool()` 仍兼容返回纯文本；`ToolBatchExecutor` 对 retryable 失败默认重试一次，仍保持批次并发与结果保序；`ToolRegistry.recentToolTraceEvents()` 保留最近工具 trace，用于本地诊断和评测入口。
